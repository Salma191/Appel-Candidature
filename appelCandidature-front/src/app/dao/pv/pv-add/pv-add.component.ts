...
@Component({
  ...
  styleUrls: ['./pv-add.component.scss'],
...
export class PvAddComponent {
  ...
  onTypeChange() {
    if (this.selectedType) {
      this.getPostesByTypeAndEntite();
    }
  }

  getPostesByTypeAndEntite() {
    if (!this.selectedType && !this.selectedEntite) {
      console.warn("Aucun type ou entite sélectionné !");
      return;
    }
    console.log("Type sélectionné :", this.selectedType, "Entite sélectionnée :", this.selectedEntite);
    this.pvService.getPostesByTypeAndEntite(this.selectedEntite,this.selectedType).subscribe(
      (data: any[]) => {
        this.postes = data;
